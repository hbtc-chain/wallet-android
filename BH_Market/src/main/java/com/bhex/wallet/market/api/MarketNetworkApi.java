...
public class MarketNetworkApi extends NetworkApi {
    ...
    private static volatile MarketNetworkApi INSTANCE;

    public static MarketNetworkApi getInstance(){

        if(INSTANCE==null){
            synchronized (MarketNetworkApi.class){
                if(INSTANCE==null){
                    INSTANCE = new MarketNetworkApi();
                }
            }
        }
        return INSTANCE;
